...
const CACHE='havoc-hub-v3';
const HEADS='havoc-heads-v1'; // HockeyTech headshots — cache-first so packets build offline
self.addEventListener('install',e=>{
  e.waitUntil(caches.open(CACHE).then(c=>c.addAll(['./','./index.html','./mobile.css','./data/bios.json','./data/havoc_players.json'])).then(()=>self.skipWaiting()));
});
