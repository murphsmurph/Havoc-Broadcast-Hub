...
const CACHE='havoc-hub-v6';
const HEADS='havoc-heads-v1'; // HockeyTech headshots — cache-first so packets build offline
self.addEventListener('install',e=>{
  e.waitUntil(caches.open(CACHE).then(c=>c.addAll(['./','./index.html','./mobile.css','./js/career-math.js','./js/daily-report-parser.js','./data/bios.json','./data/havoc_players.json','./data/matchups.json','./data/season2526_final.json'])).then(()=>self.skipWaiting()));
});
